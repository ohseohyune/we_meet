...
"""
This module provides mathematical functions and utilities for DREAM Lab.
...
def test_cubic_plot():
    ...
    x_0 = 0.0 # start pos
    x_f = 1.0 # end pos
    x_dot_0 = 0.0 # start speed
    x_dot_f = 0.0 # end speed

    positions = [cubic_spline(t_i, 0.0, 1.0, x_0, x_f, x_dot_0, x_dot_f) for t_i in t]
...
def test_cubic_multi_plot():
    import matplotlib.pyplot as plt

    t = np.linspace(0, 1, 100)
    x_0 = np.array([0.0, 1.0]) # start pos
    x_f = np.array([1.0, -1.0]) # end pos
    x_dot_0 = np.array([0.0, 0.0]) # start speed
    x_dot_f = np.array([0.0, 0.0]) # end speed

    positions = [cubic_spline(t_i, 0.0, 1.0, x_0, x_f, x_dot_0, x_dot_f) for t_i in t]
    positions = np.array(positions)

    plt.plot(t, positions[:, 0], label='X-axis')
    plt.plot(t, positions[:, 1], label='Y-axis')
    plt.title("Multi-variable Cubic Spline Interpolation")
    plt.xlabel("Time")
    plt.ylabel("Position")
    plt.grid()
    plt.legend()
