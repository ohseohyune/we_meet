...
SEGMENTS = (
    {
        "name": "top_to_bottom_ccw",
        "description": "Segment 1: 12시(top) -> 6시(bottom), CCW",
        "phi0": np.pi / 2.0,
        "phif": 3.0 * np.pi / 2.0,
    },
    {
        "name": "bottom_to_top_cw",
        "description": "Segment 2: 6시(bottom) -> 12시(top), CW",
        "phi0": 3.0 * np.pi / 2.0,
        "phif": np.pi / 2.0,
    },
    {
        "name": "top_to_bottom_cw",
        "description": "Segment 3: 12시(top) -> 6시(bottom), CW",
        "phi0": np.pi / 2.0,
        "phif": -np.pi / 2.0,
    },
    {
        "name": "bottom_to_top_cw_final",
        "description": "Segment 4: 6시(bottom) -> 12시(top), CW",
        "phi0": -np.pi / 2.0,
        "phif": np.pi / 2.0,
    },
)
...
def inside_orientation(position: np.ndarray, target: np.ndarray) -> np.ndarray:
    """Orientation whose end-effector +z axis points toward the inside target."""
    z_axis = np.asarray(target, dtype=float) - np.asarray(position, dtype=float)
    z_axis /= np.linalg.norm(z_axis) + 1e-12

    up_hint = np.array([0.0, 0.0, 1.0])
    if abs(np.dot(z_axis, up_hint)) > 0.98:
...
def total_segment_angle() -> float:
    """Total absolute angular travel over the requested half-circle segments."""
...
def segmented_circle_trajectory(
    center: tuple[float, float, float] | np.ndarray = (0.37, 0.0, 0.57),
    radius: float = 0.1652173913,
    segment_duration: float = 9.0,
    dt: float = 0.01,
    yaw: float = 0.0,
    orientation_target: tuple[float, float, float] | np.ndarray = (0.55, 0.0, 0.57),
    target_radius: float = 0.03025,
) -> dict:
    """
    ...
    Segment definitions:
      1. top -> bottom, CCW:  pi/2  ->  3pi/2
      2. bottom -> top, CW:   3pi/2 ->  pi/2
      3. top -> bottom, CW:   pi/2  -> -pi/2
      4. bottom -> top, CW return in the viewer/front convention: -pi/2 -> pi/2
    """
...
def multi_ring_segmented_trajectory(
    seam_center: tuple[float, float, float] | np.ndarray = (0.55, 0.0, 0.57),
    ring_specs=DEFAULT_MULTI_RING_SPECS,
    segment_duration: float = 9.0,
    dt: float = 0.01,
    target_radius: float = 0.03025,
